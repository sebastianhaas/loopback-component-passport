...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  switch (authType) {
    ...
    case 'facebook-token':
      ...
      strategy = new AuthStrategy(_.defaults({
        clientID: clientID,
        clientSecret: clientSecret,
        passReqToCallback: true // Tell FacebookTokenStrategy to pass the request on
      }, options),
      function(req, accessToken, refreshToken, profile, done) {
        // The user identity is matched using all fields, so this login must match
        // the original authentication scheme and provider name.
        var originalAuthScheme = 'oAuth 2.0';
        var originalProvider = 'facebook';
    ...
    break;
    case 'google-token':
      AuthStrategy = require(options.module).Strategy;
      strategy = new AuthStrategy(_.defaults({
          clientID: clientID,
          clientSecret: clientSecret,
          passReqToCallback: true // Tell GoogleTokenStrategy to pass the request on
        }, options),
        function(req, accessToken, refreshToken, profile, done) {
          // The user identity is matched using all fields, so this login must match
          // the original authentication scheme and provider name.
  ...
  if (authType === 'local') {
    ...
    self.app.post(authPath, function (req, res, next) {
      passport.authenticate(name, function (err, user, info, status) {
        // what is going on here? why don't we get proper errors here
        ...
        if (err) {
          ...
          if (info.message === 'You should provide access_token') {
            res.sendStatus(422);
          } else {
            res.sendStatus(400);
          }
